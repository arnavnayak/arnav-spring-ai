...
public class SpringAIRagServiceImpl implements SpringAIRagService {
    ...
    public ResponseEntity<String> getDocumentInformation(Question question, String userName) {
        //Commenting below as the new advisor for RAG in SpringAIChatMemoryChatClientConfig will take care of it
//        SearchRequest searchRequest = SearchRequest.builder()
//                .query(question.question())
//                .topK(3)
//                .similarityThreshold(0.5)
...
//                .collect(Collectors.joining(System.lineSeparator()));
        ...
        String answer = chatClient.prompt()
//                .system(promptSystemSpec -> promptSystemSpec.text(systemPromptHRDocumentTemplate)
//                        .param("documents",similarContext))
                .user(question.question())
                .advisors(advisorSpec -> advisorSpec.param(CONVERSATION_ID,userName))
                .call().content();
